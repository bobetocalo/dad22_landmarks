...
def draw_landmarks(predictions: Dict[str, torch.Tensor], image: np.ndarray) -> np.ndarray:
    image = draw_points(image, predictions)
    return image
...
def draw_pose(predictions: Dict[str, torch.Tensor], image: np.ndarray) -> np.ndarray:
    ...
    cv2.arrowedLine(image, (int(tdx), int(tdy)), (int(x1), int(y1)), (0, 0, 255), int(image.shape[0] * 0.008))
    cv2.arrowedLine(image, (int(tdx), int(tdy)), (int(x2), int(y2)), (0, 255, 0), int(image.shape[0] * 0.008))
    cv2.arrowedLine(image, (int(tdx), int(tdy)), (int(x3), int(y3)), (255, 0, 0), int(image.shape[0] * 0.008))
...
def get_flame_params2(predictions: Dict[str, torch.Tensor], *args: Any) -> Dict[str, List[float]]:
    params_3dmm = predictions['3dmm_params']
    flame_params = FlameParams.from_3dmm(params_3dmm, FLAME_CONSTS)
    return flame_params
